...
interface PremiumPromptToastProps {
    isOpen: boolean;
    onClose: () => void;
    onUpgrade: () => void;
}
...
const PremiumPromptToast: React.FC<PremiumPromptToastProps> = ({ isOpen, onClose, onUpgrade }) => {
    useEffect(() => {
        if (isOpen) {
    ...
    return (
        <div className="absolute top-24 left-1/2 -translate-x-1/2 w-full max-w-lg p-2 z-40">
            <div className="animate-toast bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-3 flex items-center justify-between space-x-4">
                <div className="flex-shrink-0 text-xl"><CrownIcon className="h-6 w-6 text-primary-amber" /></div>
...
export default PremiumPromptToast;